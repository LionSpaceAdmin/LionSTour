...
async function writeAudit(action: string, targetType: string, targetId: string, actorId: string, metadata?: Record<string, unknown>) {
...
export async function toggleExperienceActive(id: string, isActive: boolean) {
  ...
  if (!admin) {
    ...
    await writeAudit('toggle_experience', 'experience', id, actorId, { isActive });
...
export async function toggleExperienceFeatured(id: string, isFeatured: boolean) {
  const actorId = await requireAdmin();
  const admin = getSupabaseAdmin();
  if (!admin) {
    const updated = await prisma.experience.update({ where: { id }, data: { isFeatured } });
    await writeAudit('toggle_experience_featured', 'experience', id, actorId, { isFeatured });
    return { id: updated.id, isFeatured: updated.isFeatured };
  }
  const { error } = await admin.from('experiences').update({ is_featured: isFeatured }).eq('id', id);
  if (error) throw new Error(error.message);
  await writeAudit('toggle_experience_featured', 'experience', id, actorId, { isFeatured });
