...
import type { Prisma, Knowledge } from "@prisma/client";

export const runtime = "nodejs";

const openai = createOpenAI({ apiKey: process.env.OPENAI_API_KEY });
const gateway = null as any; // Gateway disabled for now
...
export async function POST(req: Request) {
  ...
  const rawModelId: string | undefined =
  ...
  const result = await streamText({
    ...
    tools: {
      search_experiences: defineTool({
        description:
          "Search available experiences by query, category, location, and max price.",
        parameters: z.object({
        ...
        execute: async ({
          ...
          const where: Prisma.ExperienceWhereInput = {
            isActive: true,
          } as Prisma.ExperienceWhereInput;
          if (category)
      ...
      book_experience: defineTool({
        description:
          "Create a booking for an experience when the user provides email, experienceId, date, and guests.",
        parameters: z.object({
      ...
      rag_search: defineTool({
        description:
          "Search internal knowledge base for relevant information about LionSTour and tourism platform details.",
        ...
        execute: async ({ query }: { query: string }) => {
          ...
          const qNorm = Math.sqrt(
            qVec.reduce((s: number, v: number) => s + v * v, 0)
          );
          const ranked = docs
            .map((d: Knowledge) => {
              ...
              const vNorm =
                Math.sqrt(v.reduce((s, val) => s + val * val, 0)) || 1;
