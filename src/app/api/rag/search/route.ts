...
const _gateway = process.env.AI_GATEWAY_API_KEY
...
export async function GET(req: Request) {
  ...
  if (!query) return okJson({ results: [] });
  ...
  if (supaUrl && supaAnon) {
    ...
    if (!error && Array.isArray(data)) {
      return okJson({ results: data });
    }
  }

  // Fallback to Prisma knowledge
  const qNorm =
    Math.sqrt(qVec.reduce((s: number, v: number) => s + v * v, 0)) || 1;
  ...
  const ranked = docs
    ...
    .slice(0, 5);
  return okJson({ results: ranked, target: "prisma" });
}
